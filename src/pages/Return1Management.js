...
const Return1Management = () => {
    ...
    const filterData = () => {
        if (!searchTerm) {
            setFilteredList(return1List);
        } else {
            const filtered = return1List.filter(item =>
    ...
    const handleDetailChange = (index, field, value) => {
        ...
        if (field === 'QTY' || field === 'UNIT_COST') {
            ...
            newDetails[index].AMT = Return1Service.calculateLineAmount(qty, unitCost);
        }
    ...
    const addDetailRow = () => {
        setDetails([...details, Return1Service.createEmptyDetail()]);
    };
    ...
    const resetForm = () => {
        ...
        setDetails([Return1Service.createEmptyDetail()]);
        setEditingItem(null);
    };

    const generateRefno = async () => {
        try {
            const response = await Return1Service.generateRefno(
                headerData.MYEAR,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const formattedData = Return1Service.formatReturn1Data(dataToSave, details);
    ...
    const handleEdit = async (item) => {
        try {
            ...
            if (response.success && response.data) {
                ...
                setDetails(detailsData.length > 0 ? detailsData : [Return1Service.createEmptyDetail()]);
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await Return1Service.deleteReturn1(refno);
    ...
    const handlePrint = async (item) => {
        try {
            const response = await Return1Service.getReturn1ByRefno(item.REFNO);
            if (response.success && response.data) {
                Return1Service.printReturn1(response.data.header, response.data.details);
            }
        } catch (error) {
    ...
    const calculateTotal = () => {
        return Return1Service.calculateTotal(details);
    };
            ...
            <Container maxWidth="lg" sx={{ mt: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                    <Typography variant="h5" fontWeight="bold">
                        {currentView === "add" ? "สร้างใบคืนสินค้า" : "แก้ไขใบคืนสินค้า"}
                    </Typography>
                ...
                <Card sx={{ mb: 2 }}>
                    <CardContent>
                        <Typography variant="h6" sx={{ mb: 2, color: '#5698E0' }}>ข้อมูลหัวใบคืนสินค้า</Typography>
                ...
                <Card>
                    <CardContent>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                            <Typography variant="h6" sx={{ color: '#5698E0' }}>รายละเอียดการคืน</Typography>
                                ...
                                <CardContent>
                                    ...
                                    <Grid container spacing={2}>
                                        ...
                                        <Grid item xs={12} md={2}>
                                            ...
                                            <TextField size="small" value={Return1Service.formatCurrency(detail.AMT)} disabled fullWidth
                                                sx={{ "& .MuiOutlinedInput-root": { borderRadius: "10px", backgroundColor: "#f5f5f5" } }} />
                                        </Grid>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1, mb: 2 }}>
                            <Typography variant="body1">
                                รวมเป็นเงิน: {Return1Service.formatCurrency(totals.total)} บาท
                            </Typography>
                            <Typography variant="body1">
                                VAT {headerData.VAT1}%: {Return1Service.formatCurrency(totals.vamt)} บาท
                            </Typography>
