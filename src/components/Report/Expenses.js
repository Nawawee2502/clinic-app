...
const SummaryReport = () => {
    ...
    if (incomeTypeFilter) {
      result = result.filter((item) => {
        ...
        if (incomeTypeFilter === 'บัตรทอง') {
          // ถ้ามี UCS_CARD = 'Y' หรือ PAYMENT_METHOD = 'บัตรทอง' หรือไม่มี PAYMENT_METHOD (บัตรทองไม่ต้องชำระเงิน)
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (item) =>
          ...
          item.HNCODE?.toLowerCase().includes(search)
      );
    }
    return result;
  }, [incomeRecords, startDate, endDate, incomeTypeFilter, searchTerm]);
  ...
  const filteredExpense = useMemo(() => {
    ...
    return result;
  }, [expenseRecords, startDate, endDate, searchTerm]);
  ...
  const incomeTypeOptions = useMemo(() => {
    // ✅ รวมประเภทจาก TYPE_INCOME_NAME และ TYPE_PAY
    const types = new Set();
    incomeRecords.forEach(item => {
      if (item.TYPE_INCOME_NAME) types.add(item.TYPE_INCOME_NAME);
      if (item.TYPE_INCOME) types.add(item.TYPE_INCOME);
      if (item.TYPE_PAY) types.add(item.TYPE_PAY);
    });
